...
// ── 카테고리 색상 (알 수 없는 카테고리는 팔레트에서 자동 할당) ──────────────
...
function resolveCatColor(cat: string, map: Record<string, string>): string {
...
export default function PageInventory() {
  ...
  // ── 카테고리 색상 맵 (데이터 기반 동적 생성) ────────────────────────────
  const catColors = useMemo(() => {
    const map: Record<string, string> = { ...BASE_CAT_COLORS }
    for (const item of skuList) {
      if (!map[item.category]) map[item.category] = resolveCatColor(item.category, map)
    }
    return map
  }, [skuList])
  ...
  const filtered = skuList
    .filter(i => (catF === '전체' || i.category === catF) &&
                 (i.sku.includes(search) || i.name.includes(search)))
    .sort((a, b) => sortDesc
  ...
  const SortIcon = ({ k }: { k: string }) =>
    sortKey === k ? (sortDesc ? '↓' : '↑') : <span style={{ color: T.text3, fontSize: 9 }}>↕</span>
  ...
  if (loading) {
    return (
      <div>
        ...
        <div style={{ ...card, textAlign: 'center', padding: 60, color: T.text3, fontSize: 13 }}>
          데이터 불러오는 중…
        </div>
      </div>
    )
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
        {[
          ...
          { label: '총 재고 금액',     value: `₩${(totalValue / 1_000_000).toFixed(1)}M`, sub: '현재 재고 × 단가 기준', color: T.green,  icon: '💰' },
            ...
            {cats.map(c => (
              <button key={c} onClick={() => setCatF(c)}
                style={{ fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 6,
                  border: `1px solid ${catF === c ? T.blue : T.border}`,
                  background: catF === c ? T.blueSoft : 'transparent',
                  color: catF === c ? T.blue : T.text2, cursor: 'pointer' }}>
                {c}
              </button>
            ))}
          </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            ...
            <tbody>
              {filtered.map(item => {
                ...
                const maxStock = Math.max(...skuList.map(x => x.stock), 1)
                return (
                  <tr key={item.sku}
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={9} style={{ padding: '40px', textAlign: 'center', color: T.text3, fontSize: 12 }}>
                    {skuList.length === 0 ? '재고 데이터가 없습니다.' : '검색 결과가 없습니다.'}
                  </td>
                </tr>
              )}
            </tbody>
