...
// ─── GPT 요약 (executive-report와 동일 로직) ─────────────────────────────────
async function callGPT(data: {
  targetWeek: string; weekStart: string; weekEnd: string
  weekOrderQty: number; changeRate: string | null; coverageDays: number
...
async function fetchReportData() {
  ...
  // 2. weekly_product_summary (생산량, 금액, Top5 품목)
  ...
  return {
    ...
    weekEnd: latest.end,
    weekOrderQty:    Math.round(latest.qty),
    ...
    topActions,
    forecastAccuracy,
    gptReport,  // null이면 GPT 실패, 섹션 숨김 처리
  }
}

// ─── HTML 이메일 템플릿 ───────────────────────────────────────────────────────
function buildHtml(d: Awaited<ReturnType<typeof fetchReportData>>) {
  ...
  const fmtAmt = (n: number) => n > 0 ? `${(n / 100000000).toFixed(1)}억` : '—'

  // KPI 카드 1개 생성 헬퍼
  const kpiCard = (label: string, value: string, unit: string, sub?: string) =>
    `<td width="33%" valign="top" style="padding:4px;">
      <table width="100%" height="100%" cellpadding="0" cellspacing="0" border="0" style="background:#ffffff;border:1px solid #E2E8F0;border-radius:8px;height:100%;">
        <tr><td valign="top" style="padding:14px 16px;">
          ...
          <div style="font-size:11px;margin-top:6px;min-height:16px;">${sub ?? '&nbsp;'}</div>
        </td></tr>
      </table>
    </td>`
  ...
  const riskHtml = Object.entries(d.riskSummary)
    .filter(([, c]) => c > 0)
    .map(([g, c]) => `<span style="margin-right:12px;margin-bottom:6px;display:inline-block;">
      <span style="font-size:12px;font-weight:800;color:${riskGradeColors[g] ?? '#64748B'};background:${riskGradeColors[g] ?? '#64748B'}22;border:1px solid ${riskGradeColors[g] ?? '#64748B'}44;border-radius:4px;padding:1px 7px;">${g}등급</span>
  ...
  const actionsHtml = d.topActions.length > 0
    ? d.topActions.map(a => `
      <tr>
        <td style="padding:8px 12px;border-bottom:1px solid #E2E8F0;font-size:12px;color:#1E293B;font-weight:500;">${a.product_id}</td>
        <td style="padding:8px 12px;border-bottom:1px solid #E2E8F0;font-size:12px;color:#475569;">${a.action_type}</td>
        <td style="padding:8px 12px;border-bottom:1px solid #E2E8F0;">
          <span style="font-size:10px;font-weight:700;color:${a.priority === 'critical' ? '#EF4444' : '#F59E0B'};background:${a.priority === 'critical' ? '#FEF2F2' : '#FFFBEB'};padding:2px 7px;border-radius:4px;">${a.priority.toUpperCase()}</span>
        </td>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-bottom:1px solid #E2E8F0;background:#FAFBFF;">
    <tr><td style="padding:20px 32px;">
      <div style="font-size:11px;font-weight:700;color:#6366F1;letter-spacing:1px;margin-bottom:10px;">AI 경영 요약</div>
      <div style="font-size:13px;color:#1E293B;line-height:1.7;margin-bottom:12px;">${d.gptReport.summary}</div>
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#ffffff;border:1px solid #E2E8F0;border-radius:8px;margin-bottom:10px;">
        <tr><td style="padding:14px;">
          <div style="font-size:10px;font-weight:700;color:#94A3B8;letter-spacing:1px;margin-bottom:8px;">주요 변화</div>
          ${d.gptReport.changes.map((c: string) => `<div style="font-size:12px;color:#475569;margin-bottom:6px;">${c}</div>`).join('')}
        </td></tr>
      </table>
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#ffffff;border:1px solid #E2E8F0;border-radius:8px;margin-bottom:10px;">
        <tr><td style="padding:14px;">
          <div style="font-size:10px;font-weight:700;color:#EF4444;letter-spacing:1px;margin-bottom:8px;">리스크</div>
          ${d.gptReport.risks.map((r: string) => `<div style="font-size:12px;color:#475569;margin-bottom:6px;">${r}</div>`).join('')}
        </td></tr>
      </table>
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#ffffff;border:1px solid #E2E8F0;border-radius:8px;">
        <tr><td style="padding:14px;">
          <div style="font-size:10px;font-weight:700;color:#10B981;letter-spacing:1px;margin-bottom:8px;">권고 액션</div>
          ${d.gptReport.actions.map((a: string) => `<div style="font-size:12px;color:#475569;margin-bottom:6px;">${a}</div>`).join('')}
        </td></tr>
      </table>
    </td></tr>
  </table>` : ''
...
<title>주간 임원 보고서</title>
</head>
<body style="margin:0;padding:0;background:#F1F5F9;font-family:'Apple SD Gothic Neo','Noto Sans KR',Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#F1F5F9;">
<tr><td align="center" style="padding:32px 16px;">
<table width="640" cellpadding="0" cellspacing="0" border="0" style="max-width:640px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;">

  <!-- 헤더 -->
  <tr><td style="background:#1E293B;padding:28px 32px;">
    <div style="font-size:10px;font-weight:600;color:#94A3B8;letter-spacing:1.5px;margin-bottom:6px;">반도체 부품&#xB7;소재 수요예측 AI</div>
    <div style="font-size:22px;font-weight:800;color:#ffffff;margin-bottom:4px;">주간 임원 보고서</div>
  ...
  <tr><td style="padding:24px 32px;background:#F8FAFC;border-bottom:1px solid #E2E8F0;">
    <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:1px;margin-bottom:14px;">이번 주 핵심 지표</div>
  ...
  <tr><td style="padding:20px 32px;border-bottom:1px solid #E2E8F0;">
    <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:1px;margin-bottom:10px;">리스크 등급 현황</div>
    <div>${riskHtml}</div>
  </td></tr>

  <!-- 주요 수주 품목 Top5 -->
  <tr><td style="padding:20px 32px;border-bottom:1px solid #E2E8F0;">
    <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:1px;margin-bottom:12px;">주요 수주 품목 Top 5</div>
  ...
  <!-- 긴급 조치 필요 항목 -->
  <tr><td style="padding:20px 32px;border-bottom:1px solid #E2E8F0;">
    <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:1px;margin-bottom:12px;">긴급 조치 필요 항목 (Critical / High)</div>
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;border:1px solid #E2E8F0;">
      <thead>
        <tr style="background:#F8FAFC;">
          <th style="padding:8px 12px;text-align:left;font-size:10px;font-weight:600;color:#64748B;border-bottom:1px solid #E2E8F0;">품목</th>
          <th style="padding:8px 12px;text-align:left;font-size:10px;font-weight:600;color:#64748B;border-bottom:1px solid #E2E8F0;">조치 유형</th>
          <th style="padding:8px 12px;text-align:left;font-size:10px;font-weight:600;color:#64748B;border-bottom:1px solid #E2E8F0;">우선순위</th>
          <th style="padding:8px 12px;text-align:left;font-size:10px;font-weight:600;color:#64748B;border-bottom:1px solid #E2E8F0;">사유</th>
        </tr>
      </thead>
  ...
  <tr><td style="padding:20px 32px;background:#F8FAFC;text-align:center;">
    ...
    <div style="font-size:11px;color:#CBD5E1;margin-top:4px;">수신 거부는 시스템 관리자에게 문의하세요.</div>
  </td></tr>
...
// ─── 핸들러 ───────────────────────────────────────────────────────────────────
async function sendReportEmail(req: Request) {
  const auth = req.headers.get('authorization')
  if (req.method === 'GET' && process.env.CRON_SECRET && auth !== `Bearer ${process.env.CRON_SECRET}`) {
  ...
  try {
    ...
    if (!recipients || recipients.length === 0) {
      return NextResponse.json({ ok: true, message: '활성 수신자 없음', sent: 0, failed: 0 })
    }
    ...
    console.log(`[report-email] ${reportData.week} 발송: 성공 ${succeeded} / 실패 ${failed}`)

    return NextResponse.json({
      ok: true, week: reportData.week,
      sent: succeeded, failed, total: recipients.length,
      gptOk: !!reportData.gptReport,
      at: new Date().toISOString(),
    })

  } catch (err: any) {
    console.error('[report-email] 오류:', err.message)
    return NextResponse.json({ error: err.message }, { status: 500 })
  }
}

export async function GET(req: Request)  { return sendReportEmail(req) }
export async function POST(req: Request) { return sendReportEmail(req) }
