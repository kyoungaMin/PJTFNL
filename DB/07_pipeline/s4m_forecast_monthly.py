...
from config import (
    ...
    MONTHLY_FEATURE_COLS, MONTHLY_PARAM_GRID,
    MONTHLY_CV_FOLDS, TUNING_METRIC,
...
def run(tune: bool = False):
    ...
    month_to_date = dict(zip(df["year_month"], df["month_start"])) if "month_start" in df.columns else {}
    ...
    print(f"  Early Stopping: {EARLY_STOPPING_ROUNDS}라운드, CV: {MONTHLY_CV_FOLDS}-fold")
